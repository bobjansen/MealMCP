...
def test_local_mode():
    ...
    try:
        from mcp_server import (
            get_server_info, list_units, list_preferences, 
            add_preference, get_all_recipes
        )
    ...
    try:
        result = list_units()
        print(f"   Found {len(result)} units: {result[:5]}..." if len(result) > 5 else f"   Units: {result}")
    except Exception as e:
        print(f"   ❌ Error: {e}")
...
def test_remote_mode():
    ...
    from mcp_server import (
        get_server_info, list_preferences, add_preference,
