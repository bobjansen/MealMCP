...
class PostgreSQLPantryManager(PantryManager):
    """PostgreSQL implementation of the PantryManager interface."""

    def __init__(self, connection_string: str, **kwargs):
        """
        Initialize the PostgreSQL pantry manager.
        ...
        if connection_string.startswith(("postgresql://", "postgres://")):
            ...
            # Assume it's a database name for localhost
            self.connection_params["database"] = connection_string
    ...
    def delete_preference(self, preference_id: int) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "DELETE FROM preferences WHERE id = %s", (preference_id,)
                    )
                    return cursor.rowcount > 0
        except Exception as e:
            ...
            with self._get_connection() as conn:
                with conn.cursor(
                    ...
                    for row in cursor.fetchall():
                        ...
                        if row_dict.get("transaction_date"):
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute("SELECT id FROM recipes WHERE name = %s", (name,))
                    result = cursor.fetchone()
                    if not result:
                    ...
                    cursor.execute(
                        """
                        UPDATE recipes
                        SET instructions = %s, time_minutes = %s, last_modified = %s
                        WHERE id = %s
                        """,
    ...
    def set_meal_plan(self, meal_date: str, recipe_name: str) -> bool:
        """Assign a recipe to a specific date in the meal_plan table."""
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
                        "SELECT id FROM recipes WHERE name = %s", (recipe_name,)
                    )
                    result = cursor.fetchone()
                    if not result:
                        return False
                    ...
                    cursor.execute(
                        """
                        INSERT INTO meal_plan (meal_date, recipe_id)
                        VALUES (%s, %s)
                        ON CONFLICT (meal_date) DO UPDATE SET recipe_id = EXCLUDED.recipe_id
                        """,
                        (meal_date, result[0]),
                    )
                    return True
